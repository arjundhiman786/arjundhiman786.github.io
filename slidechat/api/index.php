...
$app->post('/checktoken', function () use ($app) {
    ...
    $result = $app->db->query("SELECT token FROM accounts WHERE token = $token")//pg_execute($app->db, 'checkTok', array($token));
...
$app->post('/checkForAdmin', function () use ($app) {
  ...
  //  pg_prepare($app->db, 'checkadmin', 'SELECT accounttype FROM accounts WHERE token=$1');
    ...
    $result = $app->db->query("SELECT accounttype FROM accounts WHERE token=$token");
